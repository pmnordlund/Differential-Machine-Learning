...
class BlackScholes(Model):
    ...
    def get_training_data(self,dt, amount, normals, option, center, scale, sigma=0.2, spread_method = "normal",
        ...
        upper,lower only used for unfiform grid
        """
    ...
    #old        
    def test_set(self,start,stop,num,K,sigma,tau,r,return_vega=None):
        S0 = np.linspace(start,stop,num).reshape((-1, 1))
        price = self.bs_price(S0, K,r, sigma, tau)
        delta= self.bsDelta(S0, K, sigma, tau,r)
        if return_vega is not None:
            vega = self.model_vega(S0, K, sigma, tau,r)
            return S0, price, delta,vega
        vega = None
        return S0, price, delta,vega
    ...
    def get_train_all_vars(self,normals, S0_range=1.0, sigma_range=0.2, tau_range=1.0,r_range=0.0, 
      ...
      # currenrlt only in a system that extends, cannot do e.g. tau and r only 
      ...
      if anti:
        ST_tilde = self.GBM_ST(tau=tau_range, r=r_range,normals=-normals, # - normals 
